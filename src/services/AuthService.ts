...
export class AuthService {
  ...
  async updateNotificationState(userId: string, readNotifications?: string[], deletedNotifications?: string[]): Promise<any> {
    const user = await this.userRepository.findById(userId);
    if (!user) {
      throw new CustomError(404, 'User not found');
    }
    ...
    if (currentUser) {
      ...
      if (userRole === 'admin' || userRole === 'manager' || userRole === 'team lead') {
        ...
        for (const proj of userProjects) {
          if (proj.ownerId) sharedUserIds.add(proj.ownerId.toString());
          if (proj.managerId) sharedUserIds.add(proj.managerId.toString());
          if (proj.teamLeadId) sharedUserIds.add(proj.teamLeadId.toString());
          if (proj.clientId) sharedUserIds.add(proj.clientId.toString());
          if (proj.members) {
            for (const m of proj.members) {
              if (m.userId) sharedUserIds.add(m.userId.toString());
            }
          }
        }
  ...
  async updateProfile(
    ...
    if (updateData.collaborators !== undefined) user.collaborators = updateData.collaborators;
  ...
  async inviteCollaborator(
    userId: string,
    inviteeData: { email: string; name: string; role: string; bg: string; initials: string }
  ): Promise<any> {
  ...
  async acceptCollaboration(inviterId: string, inviteeId: string): Promise<void> {
    ...
    if (inviteeCollab) {
      ...
      invitee.collaborators.push({
        ...
        role: inviter.role,
        bg: bgColors[index],
        initials: inviter.name.split(' ').map((n: string) => n[0]).join('').toUpperCase().substring(0, 2) || 'U',
        status: 'Accepted'
      });
    }
  ...
  async removeCollaborator(userId: string, colleagueEmail: string): Promise<any> {
    const user = await this.userRepository.findById(userId);
    if (!user) {
      throw new CustomError(404, 'User not found');
    }
